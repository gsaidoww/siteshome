...
import { StyleSheet, ScrollView, Text, View, Dimensions, Image, ImageBackground, TouchableOpacity } from 'react-native';
...
export default function BannerOne({ data }) {
  ...
  const next = () => {
    if(count < data.length - 1) {
      scrollviewRef.current.scrollTo({ x: scroll + windowWidth/1.833 });
      setScroll(scroll + windowWidth/1.833);
      setCount(count + 1);
    } else {
      setDirection(-1); // Если достигли конца, меняем направление на обратное
      console.log(count);
    }
  }
  ...
  const prev = () => {
    if(scroll > 0) {
      ...
      setDirection(1); // Если достигли начала, меняем направление на вперед
    }
  }
  ...
  return (
    <View style={{ width: windowWidth, justifyContent: 'center', paddingHorizontal: windowWidth/4.4 }}>
      <ScrollView ref={scrollviewRef} horizontal showsHorizontalScrollIndicator={false} style={{ zIndex: 9999 }}>
        {data && data.map((item, index) => (
          <TouchableOpacity onPress={() => navigate(`/goods/subcategory/${item.id_sub_category}/${item.title}`)} style={{ height: windowHeight/4, width: windowWidth/1.833, backgroundColor: '#f5f5f5', borderRadius: windowWidth/450 }}>
            <Image source={{ uri: item.image }} style={{ height: windowHeight/4, width: windowWidth/1.833, backgroundColor: '#f5f5f5', borderRadius: windowWidth/450 }} />
          </TouchableOpacity>
        ))}
      </ScrollView>
