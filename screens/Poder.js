...
export default function Poder() {
    ...
    const [goodser, setGoodser] = useState(JSON.parse(params.goods));
    ...
    const getGoods = async () => {
        const q = query(collection(db, "goods"), where("subcategory_id", "==", params.category));
            ...
            {loader == false ?
                <View style={{ paddingHorizontal: windowWidth/4.4, width: windowWidth }}>
                    <View style={{ flexDirection: 'row', marginTop: windowHeight/40, alignItems: 'flex-start', flexWrap: 'wrap' }}>
                        {data && data.map((item, index) => goodser.includes(item.id) ? (
